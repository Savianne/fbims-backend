...
async function updateCurrentAddress(memberUID: string, updateData: IUpdateAddressData): Promise<{ querySuccess: boolean }> {
    ...
    return new Promise<IQueryPromise>((resolve, reject) => {
        ...
        .then(connection => {
            ...
            .then(async () => {
                ...
                getFKey.current_address_out_ph && await connection.query("DELETE FROM outside_ph_address WHERE id = ?", [getFKey.current_address_out_ph])
                //DELETE Address PH
                getFKey.current_address_ph && await connection.query("DELETE FROM local_address_ph WHERE id = ?", [getFKey.current_address_ph]);
                //Insert new values
                ...
                await connection.query('UPDATE members_personal_info SET current_address_ph = NULL, current_address_out_ph = NULL WHERE id = ?', [getFKey.personal_info_id]);
                //Update new FKeys from members_personal_info table
                updateData.addressType == "local" && localAddress && await connection.query("UPDATE members_personal_info SET current_address_ph = ? WHERE id = ?", [localAddress, getFKey.personal_info_id]);
                updateData.addressType == "outside" && outsidePHAddress && await connection.query("UPDATE members_personal_info SET current_address_out_ph = ? WHERE id = ?", [outsidePHAddress, getFKey.personal_info_id]);
...
export default updateCurrentAddress;
