...
async function addPresent(congregationUID: string, categoryUID: string, entry: TEntry): Promise<{ querySuccess: boolean, error?: any, profile: {memberUID: string, name: string, picture: string | null}}> {
    ...
    return new Promise<{ querySuccess: boolean, error?: any, profile: {memberUID: string, name: string, picture: string | null}}>((resolve, reject) => {
        ...
        .then(connection => {
            ...
            .then(async () => {
                ...
                //Check if the attender already present to the entry
                const alreadyPresent = ((await connection.query("SELECT * FROM basic_attendance WHERE entry_uid = ? AND member_uid = ?", [entry.entryUID, entry.memberUID])) as RowDataPacket[][])[0].length;

                if(alreadyPresent) throw "Already Present";

                await connection.query("INSERT INTO basic_attendance (entry_uid, member_uid, entry_session) VALUES(?, ?, ?)", [entry.entryUID, entry.memberUID, entry.session]);
