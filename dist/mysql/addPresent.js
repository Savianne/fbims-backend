...
function addPresent(congregationUID, categoryUID, entry) {
        ...
        return new Promise((resolve, reject) => {
            promisePool.getConnection()
                ...
                connection.beginTransaction()
                    ...
                    const alreadyPresent = (yield connection.query("SELECT * FROM basic_attendance WHERE entry_uid = ? AND member_uid = ?", [entry.entryUID, entry.memberUID]))[0].length;
                    if (alreadyPresent)
                        throw "Already Present";
                    yield connection.query("INSERT INTO basic_attendance (entry_uid, member_uid, entry_session) VALUES(?, ?, ?)", [entry.entryUID, entry.memberUID, entry.session]);
                    //get profile
