...
function addTimeOut(congregationUID, categoryUID, entry) {
        ...
        return new Promise((resolve, reject) => {
            promisePool.getConnection()
                ...
                connection.beginTransaction()
                    ...
                    const pendingTimeOut = (yield connection.query("SELECT id FROM detailed_attendance WHERE entry_uid = ? AND member_uid = ? AND time_out IS NULL", [entry.entryUID, entry.memberUID]))[0];
                    if (pendingTimeOut.length == 0)
                        throw "NO PENDING TIME-OUT";
                    yield connection.query("UPDATE detailed_attendance SET time_out = ? WHERE entry_uid = ? AND member_uid = ? AND id = ?", [entry.timeOut, entry.entryUID, entry.memberUID, pendingTimeOut[0].id]);
                    //get profile
