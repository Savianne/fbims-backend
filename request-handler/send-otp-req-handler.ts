...
const sendOTPReqHandler = async (req:Request, res:Response) => {
    const user = {
    ...
    const otp = otpGenerator.generate(5, { upperCaseAlphabets: false, lowerCaseAlphabets: false, specialChars: false, digits: true});
    ...
    transport.sendMail(message).then(response => {
        ...
        const pendingOTPCookie = jwt.sign(OTP, access_token_secret, { expiresIn });
...
export default sendOTPReqHandler;